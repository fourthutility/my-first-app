...
test.describe('Scout report page', () => {
  test('loads /scout-report.html without crashing', async ({ page }) => {
    ...
    // hang or throw. Use .first() so the locator resolves whichever appears.
    await expect(
      page.locator('h1, .error').first()
    ).toBeVisible({ timeout: 15_000 });

    // No uncaught exceptions from SW registration, Auth0 dynamic import,
    // render functions, or any of the inline helpers.

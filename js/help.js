    ...
    "portfolio-scout": {
      ...
      sections: [
        ...
        },
        {
          ...
          body: "The status line after a scrape names the path the extractor took:<ul><li><code>haiku_html</code> — content-bearing static HTML, parsed by Claude Haiku. The modal path. Free.</li><li><code>haiku_html_headless</code> — page was a JavaScript shell; rendered via ScrapingAnt's headless browser, then parsed by Haiku. Unlocks Cousins, JBG Smith, Greystar-style React SPAs. Costs 1 ScrapingAnt request per call (check the dashboard for credit cost — varies by plan).</li><li><code>haiku_html_cloudflare_bypass</code> — Cloudflare bot challenge blocked the static fetch; routed through ScrapingAnt with a residential proxy to bypass, then parsed by Haiku. Residential calls cost more than datacenter; check the dashboard.</li><li><code>sitemap</code> — the page was a JavaScript shell; candidate URLs recovered from <code>sitemap.xml</code>. No headless cost.</li><li><code>jsonld</code> — page exposed structured <code>RealEstateListing</code> schema. Rare. Cleanest when it happens.</li></ul>"
        },
        {
          heading: "Skip reasons",
          body: "Sometimes a URL can't be processed:<ul><li><code>skip:cloudflare</code> — bot challenge. If the headless fallback is enabled the extractor tries to bypass with a residential proxy; if that also fails, this skip surfaces.</li><li><code>skip:fund_structure</code> — fund overview without a building list.</li><li><code>skip:shell_no_sitemap</code> — JavaScript shell, no usable sitemap, AND no headless fallback configured (set <code>SCRAPINGANT_API_KEY</code> on the edge function to enable it).</li><li><code>skip:no_content_after_render</code> — headless browser rendered the page successfully but it still had no extractable content. Site may load data via XHR after a delay.</li><li><code>skip:render_failed</code> — ScrapingAnt API error or timeout. Check the dashboard for credit usage.</li><li><code>skip:http_NNN</code> — bad URL or server error.</li></ul>"
        },
        {

...
## Detail-page spike (2026-05-16): management-company coverage

One detail page sampled from each of the three sites that produced the cleanest index extraction in the pre-flight matrix. Question being tested: do owner-portfolio detail pages reliably surface the **management company name** — the firm managing the asset, not the on-site contact person — so that a v1 detail-page crawl can populate `property_management_company` as a labeled field?

| Site | Detail URL | Management-firm name on the page? | Where, if so |
| --- | --- | --- | --- |
| Stiles | `https://www.stiles.com/shalimar-apartments/` | **No firm named.** "Property Management" appears only as a service-line label in the global nav. The page exposes Type / Market / Units / Address and an image — no leasing block, no operations contact, no "managed by" attribution. Stiles is implied as the owner-operator by the page footer; manager firm is inferred, never stated. | n/a |
| Granite | `https://graniteprop.com/3630-peachtree/` | **No management firm named.** "On-site property management" appears as an *amenity bullet*, not an attribution. The page does name a leasing team — internal "Leasing & Asset Manager" Fenway Merlino at `@graniteprop.com`, plus three Stream Realty brokers at `@streamrealty.com` — but leasing is not management. | leasing block lists names + emails + phones; Property Management firm itself is unlabeled. |
| Childress Klein | `https://www.childressklein.com/550-s-tryon` | **No management firm named.** Childress Klein appears as developer ("LEED Platinum office tower developed by Childress Klein") and the global nav advertises Property Management as a CK service line, but the page never says "550 S. Tryon is managed by [firm]". | n/a |

**0/3 detail pages explicitly label the management company.** All three sites are vertically-integrated firms (owner + developer + manager), and on a vertically-integrated owner's site, the manager is the publisher — but it's *structurally never written down* because from the publisher's point of view it's obvious. The page leaves the reader to infer it. Where there's third-party management (the BREIT-owned-Greystar-managed case), the BREIT page wouldn't host the detail at all — we'd hit Greystar's site, and the attribution problem the pre-flight already flagged is the bigger blocker than extraction.

Adjacent finding: leasing contacts *are* extractable — Granite's detail page exposed four named leasing reps with email + phone, distinguishing internal (`@graniteprop.com`) from external broker (`@streamrealty.com`) by domain. That's a useful field, but it's leasing, not Property Management, and the conflation between the two is exactly the operator-conversation confusion the decision log already warned against.

**Recommendation: do not architect v1 around index → detail-page → company-name extraction.** The detail-page crawl will fill SF / year-built / leasing-contact reliably (per the existing conclusions section), but Property Management as a *labeled firm* is not a field detail pages carry. Two practical paths follow:

1. For owner-published portfolios (the modal target — institutional owners on their own sites), default `property_management_company` to the *publisher organization* extracted from the source URL's root domain, with a `confidence: implied` flag. This is structurally correct ~50% of the time and surfaceable to the human reviewer for the rest.
2. For the residual (third-party-managed assets, manager-not-owner cases), Property Management firm attribution requires a **search-driven enrichment workstream** — Sonnet + web search, per-building, anchored on the parcel or street address. That's the second pipeline the decision log already deferred. This spike confirms it can't be folded into the detail-page crawl; it needs to be its own thing.

  ...
  if (req.method === "GET") {
    ...
    try {
      const sbRes = await fetch(
        `${SB_URL}/rest/v1/projects?id=eq.${projectId}&select=address,property_name,scout_brief,scout_brief_at,property_type,use_type,num_stories,building_class,total_available_sf,percent_leased,owner_developer,limited_partners`,
        { headers: { "apikey": SB_SRK, "Authorization": `Bearer ${SB_SRK}` } }
      );
